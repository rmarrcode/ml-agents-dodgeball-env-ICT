...
public class Waypoint : MonoBehaviour {
	...
	static Color whiteColor = new Color(1f, 1f, 1f, 0.05f);
	/// <summary>
	/// Main node color
	/// </summary>
	...
	public virtual void OnDrawGizmos() {
		...
		for (int i = 0; i < outs.Count; ++i) {
			...
			ForGizmo(transform.position+direction.normalized, direction -direction.normalized*2f, new Color(1f,0f,0f,0.25f), 1f);
		}
		
		if (color.Equals(Color.green) || color.Equals(whiteColor)) color = whiteColor;
	}
	...
	public virtual void OnDrawGizmosSelected() {
		...
		for (int i = 0; i < outs.Count; ++i) {
			Vector3 direction = outs[i].waypoint.transform.position-transform.position;
			ForGizmo(transform.position+direction.normalized, direction -direction.normalized*2f, new Color(1f,0f,1f,0.25f), 1f);
		}
	}
